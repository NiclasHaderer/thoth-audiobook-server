...
class AudibleClient(val region: AudibleRegions, val imageSize: Int = 500) : MetadataProvider {
  ...
  override suspend fun getAuthorByName(authorName: String): List<MetadataAuthorImpl> {
    ...
    return coroutineScope {
      FuzzySearch.extractSorted(authorName, searchResult) { it.author!!.name }
          .map { async { getAuthorByID(uniqueName, it.referent.author!!.id.itemID) } }
          .awaitAll()
  ...
  override suspend fun getBookByName(
    ...
    return coroutineScope {
      FuzzySearch.extractSorted(bookName, searchResult) { it.title }
          .map { async { getBookByID(uniqueName, it.referent.id.itemID) } }
          .awaitAll()
          .distinctBy { it?.id?.itemID }
          .filterNotNull()
    }
  ...
  override suspend fun getSeriesByName(
    ...
    return coroutineScope {
      FuzzySearch.extractSorted(seriesName, seriesResult) { it.title }
          .map { async { getSeriesByID(uniqueName, it.referent.id.itemID) } }
          .awaitAll()
          .distinctBy { it?.id?.itemID }
          .filterNotNull()
    }
