...
import io.thoth.common.extensions.tap
...
class DatabaseMigrator(
    private val db: Database, private val packageName: String = "io.thoth.database.migrations.history"
) {
    ...
    private val classes: List<Migration> by lazy {
        ...
        try {
            ...
            // Caused by hot reloading, so migrations should not be necessary, because the migration has already run
            listOf()
        }
    }
    ...
    private val migrations: List<MigrationHolder> by lazy {
        classes.tap {
            if (!it::class.java.simpleName.contains(classNameMatcher)) {
                throw IllegalStateException("Migration class name does not match the pattern: ${it::class.java.simpleName}")
            }
        }.filter { migration -> migration::class.java.simpleName.contains(classNameMatcher) }.map { migration ->
            val version = getVersionFromString(migration::class.java.simpleName)
            MigrationHolder(version, migration)
        }.sortedBy { it.version }
    }

    private fun getLatestVersion(): Int? = transaction(db = db) {
    ...
    private fun runRollback(latestDbVersion: Int) {
        SchemaTracker.find { TSchemaTrackers.version greater latestDbVersion }
            .orderBy(TSchemaTrackers.version to SortOrder.DESC).forEach {
                log.info("Rolling back migration ${it.version}")
                transaction(db = db) {
                    try {
                        it.rollback.split(Char.MIN_VALUE).forEach { statement ->
                            exec(statement)
                        }
                    } catch (e: Exception) {
                        rollback()
    ...
    private fun runMigrations(
        migrations: List<MigrationHolder>
    ) {
        migrations.forEach {
            try {
                transaction(db = db) {
    ...
    private fun saveMigrationInDatabase(migrationHolder: MigrationHolder) = transaction(db = db) {
        SchemaTracker.new {
            date = System.currentTimeMillis() / 1000L
            version = migrationHolder.version
            rollback = migrationHolder.migration.generateRollbackStatements(db)
