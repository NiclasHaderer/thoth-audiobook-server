...
internal fun RouteHandler.patchSeries(
  ...
  val series =
      Series.findById(seriesId.id) ?: serverError(HttpStatusCode.NotFound, "Could not find series")
  ...
  if (patchSeries.authors != null) {
    series.authors =
        patchSeries.authors
            .map {
              Author.findById(it) ?: serverError(HttpStatusCode.NotFound, "Could not find author")
            }
            .toSizedIterable()
  }
