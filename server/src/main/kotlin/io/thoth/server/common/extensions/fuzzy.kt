...
fun <T> SizedIterable<T>.fuzzy(query: String, getValues: (T) -> List<String>): List<T> =
    this.mapNotNull { item -> getValues(item).map { it.fuzzy(query, item) }.maxByOrNull { it.match } }
        .filter { it.matches }
